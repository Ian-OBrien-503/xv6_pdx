...
// head and tail pointers for each process state list
#ifdef CS333_P3
struct ptrs{
  struct proc *head;
  struct proc *tail;
...
// including constant statecount for ptable
#ifdef CS333_P3
...
static struct {
  ...
  // adding array of size of the number of states to build proc lists
#ifdef CS333_P3
...
{
   if(!p)
        panic("trying to remove null");
    if(!ptable.ready[p->priority].head)
        panic("null head");
    if(!ptable.ready[p->priority].tail)
        panic("null tail");
    int rc = stateListRemove(&ptable.ready[p->priority], p);
    if(rc != 0)
        panic("remove ready error");
}
